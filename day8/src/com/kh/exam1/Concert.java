...
public class Concert {
    ...
    private int costOverWeb; //인터넷 티켓 가격

    private int costAtVenue; //현장 티켓 가격

    private int totalSalesPrice; //총 판매금액

    private boolean webSalesAble = true; //인터넷 판매 가능 여부
    ...
    public Concert(String name, int max, int costOverWeb, int costAtVenue) {
        this.singerName = name; //this. 생략 가능
        this.capacity = max; //this. 생략 가능
    ...
    public void doTicketSale() {
        ...
        if (saleCompleted) {
            ...
            System.out.println("죄송합니다. 잔여수량이 부작합니다.");
            //System.out.println("잔여 수량 : "+getTicketsLeft());
        }
    }
    ...
    public boolean sellTickets(int number) {
        ...
        if (getTicketsLeft() >= number) {
            ...
            totalSalesPrice += getSaleCost(number); //총판매금액 누적
            flag = true;
        } else {
            flag = false;
